...
jest.mock("./logger");
jest.mock("./domain/history-service");
const scheduleService = require("./schedule-service");
const logger = require("./logger");
const historyService = require("./domain/history-service");
...
describe("scheduler", () => {
  ...
  it("should switch circuits when one is on", async () => {
    ...
    expect(circuitService.circuit(3).isOn).toBe(true);
    const isoDate = /\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d.*/;
    expect(historyService.addEntries).toHaveBeenCalledWith([
      expect.objectContaining({
        circuit: 3,
        startTime: expect.stringMatching(isoDate),
        endTime: expect.stringMatching(isoDate),
      }),
    ]);
  });
});
