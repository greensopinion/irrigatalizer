 ...
 * not expose the in-memory log over REST, so we reconstruct human-readable "turned
 * on / turned off" events from the recorded runs — the same transitions the
 * scheduler logs — and show the most recent first.
 */
export function ActivityLog({
  ...
  const entries = useMemo(() => {
    ...
    for (const run of runs) {
      ...
      if (run.end !== null) {
        events.push({ at: run.end, message: `${name} turned off` });
      }
    }
